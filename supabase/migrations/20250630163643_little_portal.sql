/*
  # School Domains Table and Email Validation

  1. New Tables
    - `school_domains` - Stores valid school email domains and their corresponding school names
  
  2. New Functions
    - `validate_school_email_domain` - Validates if an email belongs to a recognized school domain
    - `get_school_from_email` - Extracts school name from email domain
  
  3. Security
    - Enable RLS on the school_domains table
...
-- Create policies for the school_domains table (with existence checks)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Create trigger to update updated_at column (with existence check)
DO $$
BEGIN
  IF NOT EXISTS (
